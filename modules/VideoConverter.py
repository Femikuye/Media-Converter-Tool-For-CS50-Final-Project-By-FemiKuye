...
from modules.FfmpegWrapper import convert_video as video_converter
import io
...
video_formats = [
    'mp4', 'avi', '3gp', 'webm', 'flv', 'ogg', 'mkv', 'mov'
]

class Converter:
    def __init__(self, upload_path, scheduler):
        self.error_message = ""
        self.upload_path = f"{upload_path}/videos"
        if not os.path.exists(self.upload_path):
            os.makedirs(self.upload_path, exist_ok=True)
        self.scheduler = scheduler
    def convert_video(self, file_obj, output_format):
        ...
        do_convert = video_converter(uploaded_file, output_file)
        if do_convert["success"]:
            ...
            return {
                "file_url": f"{ROOT_URL}/download/video/{output_name}"
            }
        else: 
